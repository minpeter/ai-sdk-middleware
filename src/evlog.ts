// biome-ignore-all lint: MIT port of HugoRCD/evlog/src/ai — keep upstream structure
// biome-ignore-all assist: MIT port of HugoRCD/evlog/src/ai — keep upstream structure
// biome-ignore-all format: MIT port of HugoRCD/evlog/src/ai — keep upstream structure
/**
 ...
 * Only intentional change vs upstream: LanguageModelV3 → LanguageModelV4 (specificationVersion "v4").
...
import type { LanguageModelV4, LanguageModelV4Middleware, LanguageModelV4StreamPart } from '@ai-sdk/provider'
...
export interface AILogger {
   ...
   *
   * Accepts a `LanguageModelV4` object or a model string (e.g. `'anthropic/claude-sonnet-4.6'`).
 ...
 * ```
 */
export function createAIMiddleware(log: RequestLogger, options?: AILoggerOptions): LanguageModelV4Middleware {
...
function buildMiddleware(log: RequestLogger, options?: AILoggerOptions): LanguageModelV4Middleware {
  const state = createAccumulatorState(options)
  state._log = log
  return buildMiddlewareFromState(log, state)
}

function buildMiddlewareFromState(log: RequestLogger, state: AccumulatorState): LanguageModelV4Middleware {
  return {
